...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_updata.setOnClickListener(view -> {
            if (ed_book.length() < 1 || ed_price.length() < 1)
                Toast.makeText(MainActivity.this, "欄位請物留空", Toast.LENGTH_SHORT).show();
            else {
                try {
        ...
        btn_delete.setOnClickListener(view -> {
            if (ed_book.length() < 1 )
                ...
                try {
                    ...
                    Toast.makeText(MainActivity.this, "刪除書名" + ed_book.getText().toString() + "價格" + ed_price.getText().toString(), Toast.LENGTH_SHORT).show();
                    ed_book.setText("");
                    ed_price.setText("");
                } catch (Exception e) {
                    e.printStackTrace();
